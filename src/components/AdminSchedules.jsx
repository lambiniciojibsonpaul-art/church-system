...
function AdminSchedules() {
    ...
    useEffect(() => {
        const checkAccessAndFetch = async () => {
            ...
            if (!roleData || roleData.role !== 'admin') { navigate('/'); return; }
    ...
    const handleSubmit = async (e) => {
        ...
        if (!error) {
            ...
            // Reset form
            setFormData({ ...formData, title: '', eventDate: '', eventTime: '', description: '' });
        } else {
    ...
    if (loading) return <div className="min-h-screen bg-gray-50 flex items-center justify-center"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[#B59E74]"></div></div>;
            ...
            <main className="flex-1 max-w-7xl w-full mx-auto px-6 pt-32 pb-12">
                ...
                <div className="flex justify-between items-end mb-8">
                    <div>
                        ...
                        <p className="text-gray-500 font-serif italic mt-1">Manage upcoming masses and custom events.</p>
                    </div>
                ...
                <div className="bg-white rounded-3xl shadow-sm border border-gray-200 p-8">
                    {events.length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {events.map(ev => (
                                <div key={ev.id} className="border border-gray-100 rounded-2xl p-6 hover:shadow-md transition-shadow relative overflow-hidden group">
                                    ...
                                    <h3 className="text-xl font-serif text-gray-800 font-medium leading-tight mb-1">{ev.title}</h3>
                                    <p className="text-sm text-gray-500 font-serif italic mb-4">{ev.priest_name}</p>
                    ...
                    <div className="bg-white w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-3xl shadow-2xl relative scrollbar-hidden">
                        ...
                        <form onSubmit={handleSubmit} className="p-8 space-y-6">
                            ...
                            <div className="grid grid-cols-2 gap-6">
                                ...
                                <div className="flex flex-col gap-1">
                                    <label className="text-xs font-bold text-gray-600 uppercase">Priest *</label>
                            ...
                            <div className="grid grid-cols-2 gap-6">
                                <div className="flex flex-col gap-1">
                                    ...
                                    <input type="date" name="eventDate" required value={formData.eventDate} onChange={handleChange} className="p-3 rounded-xl border border-gray-300 outline-none" />
                                </div>
                                <div className="flex flex-col gap-1">
                                    <label className="text-xs font-bold text-gray-600 uppercase">Start Time *</label>
                                    <input type="time" name="eventTime" required value={formData.eventTime} onChange={handleChange} className="p-3 rounded-xl border border-gray-300 outline-none" />
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-3 gap-6">
                                <div className="flex flex-col gap-1 col-span-2">
                                    ...
                                    <input type="text" name="location" required value={formData.location} onChange={handleChange} className="p-3 rounded-xl border border-gray-300 outline-none" placeholder="e.g., Main Altar" />
                                </div>
                                <div className="flex flex-col gap-1">
                                    <label className="text-xs font-bold text-gray-600 uppercase">Setting</label>
                                    <div className="flex items-center gap-4 mt-2">
                                        <label className="flex items-center gap-2 text-sm"><input type="radio" name="isInside" checked={formData.isInside === true} onChange={() => setFormData({...formData, isInside: true})} className="text-[#B59E74] focus:ring-[#B59E74]" /> Sa Loob</label>
                                        <label className="flex items-center gap-2 text-sm"><input type="radio" name="isInside" checked={formData.isInside === false} onChange={() => setFormData({...formData, isInside: false})} className="text-[#B59E74] focus:ring-[#B59E74]" /> Sa Labas</label>
                                    </div>
                                </div>
                            </div>
                            ...
                            <button type="submit" disabled={submitting} className="w-full bg-[#B59E74] hover:bg-[#9c8760] text-white font-bold py-4 rounded-xl uppercase tracking-widest mt-4">
