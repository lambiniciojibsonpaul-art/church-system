...
function StaffDashboard() {
  ...
  const fetchPendingRequests = async () => {
    ...
    const results = await Promise.allSettled(
      entries.map(([, cfg]) => restSelect(cfg.table, { match: { status: "Pending" }, order: "created_at.asc", timeoutMs: 12000 }))
    );
    const merged = {};
  ...
  const confirmAccept = async () => {
    ...
    await supabase.rpc("notify_admin", {
      ...
      notif_message: `Staff approved "${reqConfig.title(acceptingRequest)}". Please review for final approval.`,
      notif_link: "/admin",
              ...
              <div className="animate-fade-in-up">
                <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4 border-b border-gray-100 pb-6">
                  ...
                  <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                    <select value={activeServiceTab} onChange={(e) => setActiveServiceTab(e.target.value)}
                      ...
                      {TAB_NAMES.map(tab => <option key={tab} value={tab}>{tab} ({(requests[tab] || []).length})</option>)}
                    </select>
                    <select value={staffSortBy} onChange={e => setStaffSortBy(e.target.value)}
                    ...
                    <div className="flex rounded-xl border border-gray-200 overflow-hidden bg-white shrink-0">
                      <button onClick={() => setStaffViewMode("card")} title="Card view"
                        className={`px-3 py-2.5 text-sm transition-colors ${staffViewMode === "card" ? "bg-[#B59E74] text-white" : "text-gray-400 hover:text-gray-600 hover:bg-gray-50"}`}>⊞</button>
                      <button onClick={() => setStaffViewMode("table")} title="Table view"
                        className={`px-3 py-2.5 text-sm transition-colors ${staffViewMode === "table" ? "bg-[#B59E74] text-white" : "text-gray-400 hover:text-gray-600 hover:bg-gray-50"}`}>≡</button>
                    </div>
                  </div>
                </div>
                ...
                {requestsLoading ? (
                  ...
                  <div className="text-center py-16 text-gray-400 italic font-serif">
                    {/* Fixed undefined staffSearch variable here */}
                    ...
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="text-[10px] text-gray-400 uppercase tracking-widest font-bold border-b border-gray-100">
                          ...
                          <th className="p-3">Key Info</th><th className="p-3">Submitted</th><th className="p-3">Actions</th>
                        </tr>
                      </thead>
                      <tbody>
                        {pendingData.map((req) => (
                          <tr id={`request-card-${req.id}`} key={`${req._tab}-${req.id}`}
                            className={`border-b border-gray-50 transition-all duration-500 ${highlightId === req.id ? "bg-[#B59E74]/10" : "hover:bg-gray-50"}`}>
                            <td className="p-3"><span className="text-[10px] font-bold uppercase tracking-wide px-2 py-0.5 rounded-md bg-[#B59E74]/10 text-[#B59E74] whitespace-nowrap">{req._tab}</span></td>
                            <td className="p-3">
                              <p className="text-sm font-medium text-gray-800">{req._config.title(req)}</p>
                              <button onClick={() => setViewingDetails({ ...req, request_type: req._tab, display_name: req._config.title(req) })}
                                className="text-[10px] font-bold uppercase tracking-widest text-[#B59E74] hover:text-[#9c8760] transition-colors mt-0.5">Details →</button>
                            </td>
                            <td className="p-3">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {pendingData.map((req) => (
                      <div id={`request-card-${req.id}`} key={`${req._tab}-${req.id}`}
                        ...
                        <div className="mb-4">
                          ...
                          <button onClick={() => setViewingDetails({ ...req, request_type: req._tab, display_name: req._config.title(req) })}
                            className="text-[10px] font-bold uppercase tracking-widest text-[#B59E74] hover:text-[#9c8760] transition-colors mt-1">View Full Details →</button>
                        </div>
